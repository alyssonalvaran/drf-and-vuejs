...
class ProfileViewSetTestCase(APITestCase):
	...
	def test_profile_detail_update_by_owner(self):
		response = self.client.put(reverse("profile-detail", kwargs={"pk": 1}),
									{"city": "some_city", "bio": "some bio"})
		self.assertEqual(response.status_code, status.HTTP_200_OK)
	...
	def test_profile_detail_update_by_random_user(self):
		random_user = User.objects.create_user(username="random", 
											password="some_strong_pw")
		self.client.force_authenticate(user=random_user)
		response = self.client.put(reverse("profile-detail", kwargs={"pk": 1}),
									{"city": "another_city", "bio": "another bio"})
		self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class ProfileStatusViewSetTestCase(APITestCase):
	...
	def test_status_create(self):
		data = {"status_content": "new content"}
		response = self.client.post(self.url, data)
		self.assertEqual(response.status_code, status.HTTP_201_CREATED)
		self.assertEqual(response.data["user_profile"], "testcase")
		self.assertEqual(response.data["status_content"], "new content")
	...
	def test_status_update_owner(self):
		data = {"status_content": "another content"}
		response = self.client.put(reverse("status-detail", kwargs={"pk": 1}),
								data=data)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertEqual(response.data["status_content"], "another content")

	def test_status_update_random_user(self):
		random_user = User.objects.create_user(username="random", 
											password="some_strong_pw")
		self.client.force_authenticate(user=random_user)
		data = {"status_content": "invalid content"}
		response = self.client.put(reverse("status-detail", kwargs={"pk": 1}),
								data=data)
		self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
